...
class BarangMasuk extends CI_Controller
{
    ...
    private function _validasi()
    {
        ...
        $this->form_validation->set_rules('nama_supplier', 'supply', 'required');
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $kode = 'BM-' . date('ymd');
            $kode_terakhir = $this->admin->getMax('barang_masuk', 'id_kain_masuk', $kode);
            $kode_tambah = substr($kode_terakhir, -5, 5);
            $kode_tambah++;
            $number = str_pad($kode_tambah, 5, '0', STR_PAD_LEFT);
            $data['id_kain_masuk'] = $kode . $number;
    ...
    public function delete($getId)
    {
        ...
        if ($this->admin->delete('barang_masuk', 'id_kain_masuk', $id)) {
